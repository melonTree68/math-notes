...
\begin{exer}
	设数列$\{x_n\}$满足$x_n+x_m-1\leq x_{n+m}\leq x_n+x_m+1$．求证：$\frac{x_n}{n}$收敛．
\end{exer}
...
\begin{thm}[Guldinus定理]
	...
	Guldinus第一定理：位于右半平面内的平面曲线绕$y$轴旋转一周所产生的旋转曲面的面积等于质心绕$y$轴一周所经过的路程乘以曲线的弧长, 即$S_y=2\pi x_cl$．

	Guldinus第二定理：位于右半平面内的平面图形绕$y$轴旋转一周所产生的旋转立体的体积等于质心绕$y$轴一周所经过的路程乘以图形的面积．即$V_y=2\pi x_cS$．
\end{thm}
...
\begin{exer}
	...
	\begin{enumerate}
		\begin{multicols}{2}
			\item $\int_{0}^{\pi/2}\frac{\arcsin x}{x}\,\d x$．
			\item[(3)] $\int_{0}^{\pi/2}\ln\left|\sin^2x-a^2\right|\,\d x\,(a^2\leq 1)$．
			\item[(2)] $\int_{0}^{\pi/2}x\cot x\,\d x$
		\end{multicols}
	\end{enumerate}
\end{exer}
\begin{sol}
