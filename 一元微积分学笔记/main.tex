...
\begin{thm}[Sapagof判别法]
	Sapagof判别法有几种等价形式．
	\begin{enumerate}
		\item 设正数数列$\{a_n\}$单调递减．则$\lim_{\nti}a_n=0$的充分必要条件是正项级数$\series\bra{1-\frac{a_{n+1}}{a_n}}$发散．\vspace{-.5em}
		\item 设正数数列$\{a_n\}$单调递增．则$\{a_n\}$与$\series\bra{1-\frac{a_n}{a_{n+1}}}$同敛散．\vspace{-.5em}
...
\begin{pf}
	...
	\begin{enumerate}
		...
		\item $p>q$时相当于提前拿出了前$2qn$项之后的正项, 它们\qedhere
	\end{enumerate}
\end{pf}
...
\begin{exer}
	...
	\begin{enumerate}
		\begin{multicols}{2}
			\item $\series\frac{(-1)^n2^\frac{1}{n}}{\sqrt{n}}.$
			\item $\series\frac{\sin nx}{n}\bra{1+\frac{1}{n}}^n.$
		\end{multicols}
	\end{enumerate}
\end{exer}
