...
% \begin{proof}
% 	\begin{enumerate}
% 		\item We prove that $P=P_{\im P}$. We have $\ker P=(\im P)^\perp$. If $v\in\ker P$, then $Pv=0=P_{\im P}\,v$. If $v\in\im P$, suppose $v=Pu$ with $u\in V$. $P^2u=Pu$ implies that $Pv=v=P_{\im P}\,v$. $P$ and $P_{\im P}$ agree on $(\im P)^\perp=\ker P$ and $\im P$, and thus are equal.
% 		\item We prove that $P=P_{(\ker P)^\perp}$. If $v\in\ker P$, then $Pv=0=P_{\,\ker P}\,v$. If $v\in(\ker P)^\perp$, $P^2v=Pv$ implies that $Pv-v\in\ker P$, which implies that $Pv=v+w$ with $w\in\ker P$. Because $v\in(\ker P)^\perp$, $\norm{Pv}^2=\norm{v}^2+\norm{w}^2$ by the Pythagorean theorem. Then $\norm{Pv}\leq\norm{v}$ leads to $w=0$, which implies that $Pv=v$. $P$ and $P_{(\ker P)^\perp}$ agree on $\ker P$ and $(\ker P)^\perp$, and thus are equal.
% 	\end{enumerate}
% \end{proof}
...
\begin{thms}
	\begin{enumerate}
		\item Suppose $T\in\L(V)$. Suppose $\F=\R$. Then $T$ is self-adjoint if and only if all pairs of eigenvectors corresponding to distinct eigenvalues of $T$ are orthogonal and $V=E(\lambda_1,T)\oplus\cdots\oplus E(\lambda_m,T)$, where $\lambda_1,\dots,\lambda_m$ denote the distinct eigenvalues of $T$.
		\item Suppose $T\in\L(V)$. Suppose $\F=\C$. Then $T$ is normal if and only if all pairs of eigenvectors corresponding to distinct eigenvalues of $T$ are orthogonal and $V=E(\lambda_1,T)\oplus\cdots\oplus E(\lambda_m,T)$, where $\lambda_1,\dots,\lambda_m$ denote the distinct eigenvalues of $T$.
	\end{enumerate}
\end{thms}
...
\begin{exer}
	...
	\begin{enumerate}
		...
		\footnote{\idea Let $\inp{\cdot}{\cdot}_1$ be an inner product on $V$. We need to find $T$ such that $\inp{u}{v}_1=\inp{Tu}{v}$. Fixing $v$ is not sufficient to find the unique $Tu$. Thus we fix $u$ and consider $\inp{v}{u}_1=\inp{v}{Tu}$. The map $v\mapsto\inp{v}{u}_1$ is in $V'$. Hence we use the Riesz representation theorem to define $Tu$.}
	\end{enumerate}
\end{exer}
